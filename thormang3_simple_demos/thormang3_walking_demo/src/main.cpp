...
void DemoCommandCallback(const std_msgs::String::ConstPtr& msg)
{
    ...
    {
        ROS_INFO("[Demo]  : go to initial pose");
        MoveToInitPose();
        is_init_pose = true;
    ...
    {
        ROS_INFO("[Demo]  : set walking control mode");
    	SetCtrlModule();
    }
    else if( msg->data == "forward" )
    {
    	ROS_INFO("[Demo]  : forward walking");
    	WalkForward();
    }
    else if( msg->data == "backward" )
    {
    	ROS_INFO("[Demo]  : backward walking");
    	WalkBackWard();
    }
    else if( msg->data == "balance_on" )
    {
    	ROS_INFO("[Demo]  : balance enable");
    	BalanceOn();
    }
    else if( msg->data == "balance_off" )
    {
    	ROS_INFO("[Demo]  : balance disable");
    	BalanceOff();
    }
    else {
